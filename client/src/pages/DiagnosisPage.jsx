...
const DISEASE_INFO = {
  ...
  early_blight: {
    ...
    symptoms: ['Dark brown to black circular spots', 'Target-board concentric ring pattern', 'Yellow halo surrounding lesions', 'Lower leaves affected first', 'Lesions up to 1ΓÇô2 cm in diameter'],
    treatment: ['Apply chlorothalonil or mancozeb fungicide', 'Remove and destroy infected leaves immediately', 'Reduce overhead irrigation to limit moisture', 'Apply copper-based fungicide every 7ΓÇô10 days', 'Improve airflow between plants'],
  ...
  late_blight: {
    ...
    severity: 'Severe ΓÇö Act Immediately',
    ...
    treatment: ['Isolate affected plants immediately to prevent spread', 'Apply metalaxyl or cymoxanil fungicide urgently', 'Destroy severely infected plants ΓÇö do not compost', 'Alert neighbouring farms and report to admin', 'Monitor every 24 hours during outbreak'],
      ...
      {result.disease_result !== 'healthy' && (
        <div style={{ padding: '14px 18px', background: 'rgba(248,113,113,0.07)', border: '1px solid rgba(248,113,113,0.2)', borderRadius: 10, display: 'flex', gap: 12, alignItems: 'flex-start' }}>
          ...
          <div>
            ...
            <p style={{ fontSize: 12, color: 'var(--text2)', lineHeight: 1.5 }}>Disease detected on your farm could affect neighbouring farms. The Admin Panel has been notified. You can go to Admin ΓåÆ Blight Map to update your region's risk level.</p>
          </div>
        </div>
      )}
    </div>
